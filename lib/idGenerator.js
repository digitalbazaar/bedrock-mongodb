...
function _loadIds(idGenerator, callback) {
  ...
  async.waterfall([
    function(callback) {
      ...
      database.localCollection.findOne(
        query, fields,
        function(err, record) {
          if(err) {
            return callback(err);
    ...
    function(id, callback) {
      ...
      if(localId !== null) {
        ...
        return database.localCollection.update(
          query, update, database.localWriteOptions,
          function(err, result) {
            if(err) {
              return callback(err);
            }
      ...
      database.collections.distributedId.findAndModify(
        ...
        bedrock.tools.extend({}, database.writeOptions,
        ...
        function(err, result) {
          ...
          var globalId = new Id64(result.value.id);
