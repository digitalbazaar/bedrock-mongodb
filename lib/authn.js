...
export async function openDatabase(options) {
  ...
  const authRequired = await _authRequired({config});
 ...
 */
async function _authRequired({config}) {
  // determine if authN is required
  if(config.forceAuthentication) {
    return true;
  } else {
    try {
      const client = new MongoClient(config.url, config);
      await client.connect();
      const admin = client.db(config.name).admin();
      // if listing databases fails on authz, then authentication is required
      await admin.listDatabases();
    } catch(e) {
console.error('AUTH REQUIRED ERROR', e);
