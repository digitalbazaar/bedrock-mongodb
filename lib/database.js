...
function init(callback) {
  async.auto({
    ...
    initLocalDatabase: ['createIndexes', function(callback) {
      // init machine-local (non-replicated) database
      _initLocalDatabase(callback);
    }],
    emit: ['initLocalDatabase', function(callback) {
...
function initDatabase(callback) {
  ...
  async.auto({
    ...
    checkAuth: ['auth', function(callback, results) {
      ...
      console.log('\nIf the database "' + config.mongodb.name +
        '" or the user "' + config.mongodb.username + '" do not exist and ' +
      ...
      async.waterfall([
        ...
        function(collection, callback) {
          collection.findOne(
            {user: config.mongodb.username}, {user: true},
            function(err, record) {
              if(err) {
                return callback(err);
              }
        ...
        function(callback) {
          ...
          client.addUser(
            config.mongodb.username, config.mongodb.password,
            _getAddUserOptions(), function(err) {
              callback(err);
            });
        },
        function(callback) {
          // connect to local database to create user there as well
          localClient = client.db(config.mongodb.local.name);
        ...
        function(auth, callback) {
          // TODO: only execute for mongo 2.4 (or check for exception
          // and ignore local user creation error on 2.6+)
          // create database user for local database
          localClient.addUser(
            config.mongodb.username, config.mongodb.password,
            config.mongodb.writeOptions, function(err) {
              callback(err);
            });
        }
      ], function(err) {
