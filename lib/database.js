...
function init(callback) {
  async.auto({
    initDatabase: function(callback) {
      bedrock.runOnce('bedrock-mongodb.init', initDatabase, callback);
    },
    createClient: ['initDatabase', function(callback) {
      api.client = new mongo.Db(
        bedrock.config.mongodb.name, new mongo.Server(
          bedrock.config.mongodb.host, bedrock.config.mongodb.port,
          bedrock.config.mongodb.connectOptions),
          bedrock.config.mongodb.options);
      callback();
    }],
    connect: ['createClient', function(callback) {
      // connect to shared database
      logger.info('connecting to database: mongodb://' +
        bedrock.config.mongodb.host + ':' +
        bedrock.config.mongodb.port + '/' +
        bedrock.config.mongodb.name);
    ...
    auth: ['connect', function(callback) {
      api.client.authenticate(
        bedrock.config.mongodb.username,
        bedrock.config.mongodb.password, function(err) {
          callback(err);
        });
    }],
    openCollections: ['auth', function(callback) {
      // open collections
    ...
    openLocalDatabase: ['createIndexes', function(callback) {
      // connect to machine-local (non-replicated) database
      openLocalDatabase(callback);
    }],
    emit: ['openLocalDatabase', function(callback) {
...
function initDatabase(callback) {
  // TODO: tidy up code, reuse same code path w/different config for
  // shared db client vs. local db client?
  ...
  var client = new mongo.Db(
    config.mongodb.name, new mongo.Server(
      config.mongodb.host, config.mongodb.port,
      config.mongodb.connectOptions),
      config.mongodb.options);
  var localClient = new mongo.Db(
    config.mongodb.local.name, new mongo.Server(
      config.mongodb.host, config.mongodb.port,
      config.mongodb.connectOptions),
      config.mongodb.options);
  ...
  async.auto({
    ...
    authLocal: ['openLocal', function(callback) {
      localClient.authenticate(
        config.mongodb.username, config.mongodb.password,
        {authdb: config.mongodb.name}, function(err) {
          if(!err) {
            return callback(null, true);
          }
          if(api.isDatabaseError(err) && err.code === MDBE_AUTH_FAILED) {
    ...
    checkAuth: ['auth', 'authLocal', function(callback, results) {
      // authenticated, finish
      if(results.auth && results.authLocal) {
        return callback();
      }
      _createUsers(client, localClient, callback);
    }]
  }, function(err) {
 ...
 * @param callback(err) called once the operation completes.
 */
function openLocalDatabase(callback) {
  // create db client for local database
  api.localClient = new mongo.Db(
    bedrock.config.mongodb.local.name, new mongo.Server(
      bedrock.config.mongodb.host, bedrock.config.mongodb.port,
      bedrock.config.mongodb.connectOptions),
      bedrock.config.mongodb.options);
  ...
  async.waterfall([
    ...
    function(callback) {
      logger.info('connecting to database: mongodb://' +
        bedrock.config.mongodb.host + ':' +
        bedrock.config.mongodb.port + '/' +
        bedrock.config.mongodb.local.name);
    ...
    function(callback) {
      api.localClient.authenticate(
        bedrock.config.mongodb.username,
        bedrock.config.mongodb.password,
        {authdb: bedrock.config.mongodb.name}, function(err) {
          callback(err);
        });
    },
    function(callback) {
...
function _createUsers(client, localClient, callback) {
  var config = bedrock.config;
  ...
  async.auto({
    ...
    addLocalUser: ['serverInfo', 'prompt', function(callback, results) {
