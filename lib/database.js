...
function init(callback) {
  ...
  async.auto({
    ...
    openLocal: ['initDatabase', callback => {
    ...
    dropCollections: ['open', (callback, results) => {
    ...
    openCollections: ['dropCollections', callback => api.openCollections(
      ['distributedId'], callback)],
    createIndexes: ['openCollections', callback => api.createIndexes([{
      collection: 'distributedId',
      fields: {namespace: true},
      options: {unique: true, background: true}
    }], callback)],
    setupLocal: ['openLocal', (callback, results) => {
...
api.openCollections = (names, callback) => {
  ...
  // now open the collections
    ...
    names.forEach(name => {
      if(!(name in api.collections)) {
        collections[name] = (callback) => api.client.collection(name, callback);
      }
    });
...
function _setupLocalDatabase(callback) {
  ...
  async.waterfall([
    ...
    callback => api.localClient.createCollection(
      name, api.localWriteOptions, function ignoreAlreadyExists(err) {
        if(err) {
          if(api.isAlreadyExistsError(err)) {
            err = null;
          }
...
function _initDatabase(callback) {
  ...
  async.auto({
    ...
    check: ['open', 'openLocal', (callback, results) => {
...
function _openDatabase(options, callback) {
  ...
  async.auto({
    ...
    checkAuthRequired: ['connect', (callback, results) => {
    ...
    serverInfo: ['connect', (callback, results) => {
      db = results.connect;
      db.admin().serverInfo(callback);
    }],
    auth: ['serverInfo', 'checkAuthRequired', (callback, results) => {
    ...
    authSuccess: ['auth', (callback, results) => {
...
function _createUser(client, localClient, callback) {
  ...
  async.auto({
    ...
    auth: ['admin', (callback, results) => client.authenticate(
      results.admin.username, results.admin.password,
      {authdb: 'admin'}, callback)],
    serverInfo: ['auth', callback => client.admin().serverInfo(callback)],
    addLocalUser: ['serverInfo', (callback, results) => {
      ...
      async.auto({
        ...
        auth: callback => localClient.authenticate(
          ...
          callback),
        removeUser: ['auth', callback => localClient.removeUser(
        ...
        addUser: ['removeUser', callback => _addLocalUser(
    ...
    removeUser: ['auth', callback => client.removeUser(
      config.mongodb.username, err => {
        // ignore user not found
        if(api.isDatabaseError(err) && err.code === MDBE_USER_NOT_FOUND) {
          err = null;
    ...
    addUser: ['removeUser', callback => client.addUser(
