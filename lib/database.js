...
function init(callback) {
  async.auto({
    ...
    dropCollections: ['open', function(callback, results) {
      ...
      if(testMode && bedrock.config.mongodb.dropCollections.onInit) {
    ...
    auth: ['serverInfo', function(callback, results) {
      ...
      if(_usesRoles(results.serverInfo)) {
        ...
        return db.authenticate(
          config.username, config.password, opts, callback);
      }
      // backwards-compatible mode; auth using opened db
      opts = bedrock.util.extend({}, opts, {authdb: name});
      db.authenticate(config.username, config.password, opts, callback);
    }],
