...
/**
 * Creates a streaming GridFS bucket interface.
 *
 * By default the writeOptions config value is used for the GridFSBucket
 * writeConcern option.
 *
 * @param options see GridFSBucket documentation
 * @param callback(err, bucket) called once the operation completes.
 */
api.createGridFSBucket = (options, callback) => {
  let opts = options;
  if(!('writeConcern' in options)) {
    opts = Object.assign({}, options, {
      writeConcern: api.writeOptions
    });
  }
  callback(null, new mongo.GridFSBucket(api.client, opts));
};
