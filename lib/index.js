  ...
  async.auto({
    ...
    auth: ['serverInfo', 'checkAuthRequired', (results, callback) => {
      ...
      if(_usesRoles(results.serverInfo)) {
        ...
        opts = bedrock.util.extend({}, opts, {authSource: config.name});
        const client = new MongoClient(new Server(options.url), {
          user: username,
          password,
          authSource: 'adming',
          ...opts
      ...
      opts = bedrock.util.extend({}, opts, {authSource: db.databaseName});
      const client = new MongoClient(new Server(options.url), {
        user: username,
        password,
        authSource: 'adming',
