...
async function _init() {
  ...
  try {
    ...
    const {client, db} = await promisify(_openDatabase)(
      {url: config.url, init: false});
...
async function _initDatabase() {
  ...
  try {
    ...
    try {
      ({client} = await promisify(_openDatabase)({
        url: config.url,
        init: true
      }));
    } catch(e) {
      if(api.isDatabaseError(e) && (e.code === MDBE_AUTH_FAILED ||
    ...
    if(needsPrompt) {
      ...
      await promisify(_promptToCreateUser)();
    }
  } finally {
  ...
  async.auto({
    ...
    auth: ['serverInfo', 'checkAuthRequired', (results, callback) => {
      ...
      if(_usesRoles(results.serverInfo)) {
        ...
        return callbackify(_loginUser)({url, opts, auth}, callback);
      }
...
function _promptToCreateUser(callback) {
  const config = bedrock.config.mongodb;
  ...
  async.auto({
    admin: callbackify(_getAdminCredentials),
    ...
    auth: ['admin', (results, callback) => {
      ...
      callbackify(_loginUser)(
        {auth: admin.auth, opts, url: admin.url}, callback);
    }],
...
async function _dropCollections() {
  ...
  api.db.collections((skip, collections) => {
    promises.push(...collections.map(async collection => {
      // never drop `system.indexes`; otherwise, drop collection if `cArray` is
