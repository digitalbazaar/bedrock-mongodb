...
api.openCollections = (names, callback) => {
  ...
  async.each(unopened, (name, callback) => {
    ...
    api.db.createCollection(name, function ignoreAlreadyExists(err) {
      if(err) {
    ...
    names.forEach(function(name) {
      if(!(name in api.collections)) {
        collections[name] = function(callback) {
          api.db.collection(name, callback);
        };
      }
    });
...
function _openDatabase(options, callback) {
  ...
  async.auto({
    ...
    auth: ['serverInfo', 'checkAuthRequired', (results, callback) => {
      ...
      const connectOptions = config.connectOptions;
      ...
      if(_usesRoles(results.serverInfo)) {
        ...
        opts = {...opts, ...connectOptions, authSource: config.name};
        ...
        return client.connect((err, result) => callback(err, result));
      }
      ...
      client.connect((err, result) => callback(err, result));
    }],
...
function _connect(options, callback) {
  ...
  mongo.MongoClient.connect(options.url, connectOptions, (err, client) => {
    if(!err && !options.init) {
      logger.info('connecting to database: ' + options.url);
    }
