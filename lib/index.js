...
async function _init() {
  ...
  try {
    ...
    // no need to wrap another BedrockError
    if(error instanceof BedrockError) {
      throw error;
    }

    throw new BedrockError('Could not initialize database.', 'DatabaseError', {
...
async function _initDatabase() {
  ...
  try {
    await openDatabase({url: config.url, init: true});
  } catch(error) {
    if(isAuthenticationError(error)) {
      ...
      logger.error('Could not open the database.', {error});
    }
